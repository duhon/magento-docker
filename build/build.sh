...
docker run --rm --name test -it duhon/php:${ver}-${type} bash -c "git clone --depth=1 --single-branch --branch ${mage} https://github.com/magento/magento2.git /var/www/magento2ce && composer update && php -v && php -m && php -d memory_limit=-1 /var/www/magento2ce/vendor/bin/phpunit -c /var/www/magento2ce/dev/tests/unit/phpunit.xml.dist"
